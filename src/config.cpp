...
Config::Config(uint8_t cs, uint32_t size, uint32_t page, avr_mem_t memory, file_format_t format, bool verbose, bool write, const std::string &fileName)
    : cs(cs), size(size), page(page), memory(memory), format(format), verbose(verbose), write(write), fileName(fileName) {
}
